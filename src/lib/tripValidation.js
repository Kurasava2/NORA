...
export function basicTripErrors(trip, { period = null, hasMotohours = false, materialLabel = name => name } = {}) {
  ...
  if (hasMotohours && !trip?.unused) {
    const motohourStart = num(trip?.motohourStart)
    const motohourEnd = num(trip?.motohourEnd)
    const legacyMotohours = num(trip?.motohours)
    const hasNewReadings = motohourStart !== null || motohourEnd !== null
    if (hasNewReadings) {
      if (motohourStart === null || motohourEnd === null) errors.push('Не заполнены оба показания моточасов.')
      else {
        if (motohourStart < 0 || motohourEnd < 0) errors.push('Моточасы не могут быть отрицательными.')
        if (motohourEnd < motohourStart) errors.push('Конечные моточасы меньше начальных.')
      }
    } else if (legacyMotohours === null) errors.push('Не заполнены показания моточасов.')
    else if (legacyMotohours < 0) errors.push('Моточасы не могут быть отрицательными.')
  }

  const materials = Object.keys(trip?.gsm || {})
  if (!materials.length && !trip?.unused) errors.push('Не добавлен ни один тип ГСМ / масла.')
