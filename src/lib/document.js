import { fmtDate, longDate, materialCellName, materialSummaryName, nfmt, kmfmt, nonZero, num, statementMaterialNames, totals, tripMaterialNames, motohoursWorked } from './domain.js'
...
export function xlsxModel(state, p, st, v) {
  ...
  const motohourValues = trips.filter(t => !t.unused).map(t => motohoursWorked(t))
  const motohoursComplete = !v.hasMotohours || (motohourValues.length > 0 && motohourValues.every(value => value !== null))
  let km = 0
  const tripRows = trips.map((t, i) => {
    const a = num(t.odoStart), b = num(t.odoEnd)
    if (a !== null && b !== null) km += b - a
    return [i + 1, fmtDate(t.date), t.number, a, b, materialLinesText(state, t, 'start'), materialLinesText(state, t, 'received'), materialLinesText(state, t, 'spent'), materialLinesText(state, t, 'end'), v.hasMotohours ? (motohoursWorked(t) ?? '') : (a !== null && b !== null ? b - a : null), t.note || '']
  })
...
export function documentBodyHtml(state, p, st, v) {
  ...
  const motohourValues = trips.filter(t => !t.unused).map(t => motohoursWorked(t))
  const motohourTotal = motohourValues.length && motohourValues.every(value => value !== null) ? nfmt(motohourValues.reduce((sum, value) => sum + value, 0)) : '—'
  let km = 0
  trips.forEach(t => { const a = num(t.odoStart), b = num(t.odoEnd); if (a !== null && b !== null) km += b - a })
  const rows = trips.map((t, i) => {
    const a = num(t.odoStart), b = num(t.odoEnd)
    const work = v.hasMotohours ? nfmt(motohoursWorked(t)) : (a !== null && b !== null ? kmfmt(b - a) : '—')
