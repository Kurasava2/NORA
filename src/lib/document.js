import { fmtDate, longDate, materialCellName, materialSummaryName, motohoursWorked, nfmt, kmfmt, nonZero, num, statementMaterialNames, totals, tripMaterialNames } from './domain.js'
...
export function xlsxModel(state, p, st, v) {
  ...
  const workTrips = trips.filter(t => !t.unused)
  const motohourValues = workTrips.map(t => motohoursWorked(t))
  ...
  const tripRows = trips.map((t, i) => {
    ...
    const mhStart=num(t.motohoursStart),mhEnd=num(t.motohoursEnd),mh=motohoursWorked(t); return [i + 1, fmtDate(t.date), t.number, a, b, materialLinesText(state, t, 'start'), materialLinesText(state, t, 'received'), materialLinesText(state, t, 'spent'), materialLinesText(state, t, 'end'), v.hasMotohours ? (mh ?? '') : (a !== null && b !== null ? b - a : null), t.note || '', v.hasMotohours?(mhStart??''):'', v.hasMotohours?(mhEnd??''):'']
  })
  const firstMh=trips.map(t=>num(t.motohoursStart)).find(v=>v!==null)??null,lastMh=[...trips].reverse().map(t=>num(t.motohoursEnd)).find(v=>v!==null)??null; const totalRow = ['', 'ИТОГО', '', num(first?.odoStart), num(last?.odoEnd), '', '', '', '', v.hasMotohours ? (motohoursComplete ? motohourValues.reduce((sum, value) => sum + value, 0) : '') : km, '', v.hasMotohours?(firstMh??''):'', v.hasMotohours?(lastMh??''):'']
  const materialRows = statementMaterialNames(st)
    ...
    .map(m => {
      const q = tot[m]
      const shown = value => totalRaw(value)
...
export function documentBodyHtml(state, p, st, v) {
  ...
  const rows = trips.map((t, i) => {
    const a = num(t.odoStart), b = num(t.odoEnd)
    const work = v.hasMotohours ? (motohoursWorked(t)===null?'—':nfmt(motohoursWorked(t))) : (a !== null && b !== null ? kmfmt(b - a) : '—')
  ...
  const shown = value => nfmt(totalRaw(value))
