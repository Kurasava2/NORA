...
export function migrateState(input) {
  ...
  if (Array.isArray(s.vehicles)) s.vehicles = s.vehicles.map(v => { const rateType=normalizeRateType(v.rateType); return {...v, id:String(v.id || v.shortNo || uid()), shortNo:String(v.shortNo || v.id || ''), model:String(v.model || ''), reg:String(v.reg || ''), defaultMaterials:Array.isArray(v.defaultMaterials)?v.defaultMaterials:[], rateType, hasMotohours:!!v.hasMotohours || rateType===RATE_PER_MOTOHOUR} })
  else s.vehicles = clone(BASE_VEHICLES).map(v => { const x={...v, ...(s.vehicleSettings?.[v.id] || {})}; const rateType=normalizeRateType(x.rateType); return {...x,rateType,hasMotohours:!!x.hasMotohours || rateType===RATE_PER_MOTOHOUR} })
...
export function prefilledTrip(state,st,p,editingTrip=null){ if(editingTrip)return clone(editingTrip); const prev=predecessorTrip(st),gsm={},source=prev?(prev.gsm||{}):(st.opening?.gsm||{}); for(const [m,q] of Object.entries(source)){ const end=prev?num(q?.end):num(q); if(end!==null&&nonZero(state,end))gsm[m]={start:end,received:'',spent:'',end:'',_carried:true,_autoTarget:''} } const mh=prev?(motohoursEnd(prev)??''):(num(st.opening?.motohours)??''); return {id:uid(),seq:Date.now()+Math.random(),date:prev?.date||p.start,number:'',odoStart:prev?(num(prev.odoEnd)??''):(num(st.opening?.odo)??''),odoEnd:prev?(num(prev.odoEnd)??''):(num(st.opening?.odo)??''),motohoursStart:mh,motohoursEnd:mh,motohours:'',unused:false,note:'',gsm} }
