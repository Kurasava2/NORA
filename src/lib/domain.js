...
import { RATE_PER_MOTOHOUR, motohoursEnd, motohoursWorked, normalizeRateType, rateUnitLabel } from './vehicleMetrics.js'
import { defaultAutoCalcForVehicle, normalizeAutoCalc } from './autoCalc.js'
...
export { num, round2, smartBalance, toleranceOf, motohoursWorked }
...
export function migrateState(input) {
  ...
  const normalizeVehicle = inputVehicle => {
    const v={...inputVehicle},rateType=normalizeRateType(v.rateType)
    v.id=String(v.id || v.shortNo || uid())
    v.shortNo=String(v.shortNo || v.id || '')
    v.model=String(v.model || '')
    v.reg=String(v.reg || '')
    v.defaultMaterials=Array.isArray(v.defaultMaterials)?v.defaultMaterials:[]
    v.rateType=rateType
    v.hasMotohours=!!v.hasMotohours || rateType===RATE_PER_MOTOHOUR
    v.autoCalc=normalizeAutoCalc(v.autoCalc || defaultAutoCalcForVehicle(v,s.catalog))
    return v
  }
  if (Array.isArray(s.vehicles)) s.vehicles = s.vehicles.map(normalizeVehicle)
  else s.vehicles = clone(BASE_VEHICLES).map(v => normalizeVehicle({...v, ...(s.vehicleSettings?.[v.id] || {})}))
...
export function vehicleOf(state,id){ const list=state?.vehicles; const b=Array.isArray(list)?list.find(v=>v.id===id):baseVehicle(id); if(!b)return null; const o=state.vehicleSettings?.[id]; const v=o?{...b,...o}:b; const rateType=normalizeRateType(v.rateType); return {...v,rateType,hasMotohours:!!v.hasMotohours || rateType===RATE_PER_MOTOHOUR,autoCalc:normalizeAutoCalc(v.autoCalc || defaultAutoCalcForVehicle({...v,rateType},state?.catalog||[]))} }
export function vehicleRateLabel(vehicle){ const rate=num(vehicle?.baseRate); return rate===null?'по документу':`${nfmt(rate)} ${rateUnitLabel(vehicle)}` }
...
export function prefilledTrip(state,st,p,editingTrip=null){ if(editingTrip){const out=clone(editingTrip),vehicle=vehicleOf(state,st.vehicleId);out.calcInputs=out.calcInputs||{};for(const p0 of vehicle?.autoCalc?.params||[])if(out.calcInputs[p0.code]==null)out.calcInputs[p0.code]=p0.defaultValue??'';return out} const prev=predecessorTrip(st),gsm={},source=prev?(prev.gsm||{}):(st.opening?.gsm||{}); for(const [m,q] of Object.entries(source)){ const end=prev?num(q?.end):num(q); if(end!==null&&nonZero(state,end))gsm[m]={start:end,received:'',spent:'',end:'',_carried:true,_autoTarget:'',_calcSource:''} } const mh=prev?(motohoursEnd(prev)??''):(num(st.opening?.motohours)??''),vehicle=vehicleOf(state,st.vehicleId),calcInputs={};for(const p0 of vehicle?.autoCalc?.params||[])calcInputs[p0.code]=p0.defaultValue??''; return {id:uid(),seq:Date.now()+Math.random(),date:prev?.date||p.start,number:'',odoStart:prev?(num(prev.odoEnd)??''):(num(st.opening?.odo)??''),odoEnd:prev?(num(prev.odoEnd)??''):(num(st.opening?.odo)??''),motohoursStart:mh,motohoursEnd:mh,motohours:'',unused:false,note:'',calcInputs,autoCalcMeta:null,gsm} }
export function addTripMaterial(state,st,trip,name,editingTripId=null){ if(!name||trip.gsm?.[name])return trip; const prev=editingTripId?predecessorTrip(st,editingTripId):predecessorTrip(st), prevEnd=prev?num(prev.gsm?.[name]?.end):num(st.opening?.gsm?.[name]); trip.gsm=trip.gsm||{}; trip.gsm[name]={start:prevEnd!==null&&nonZero(state,prevEnd)?prevEnd:'',received:'',spent:'',end:'',_carried:prevEnd!==null&&nonZero(state,prevEnd),_autoTarget:'',_calcSource:''}; return trip }
export function reconcileCarryForward(state,st){ if(!st)return; sortTrips(st); const tol=toleranceOf(state?.settings?.tolerance); for(let i=0;i<st.trips.length;i++){ const t=st.trips[i]; t.gsm=t.gsm||{}; const source=i>0?(st.trips[i-1].gsm||{}):(st.opening?.gsm||{}); for(const [m,pq] of Object.entries(source)){ const pe=i>0?num(pq?.end):num(pq); if(pe!==null&&Math.abs(pe)>tol){if(!t.gsm[m])t.gsm[m]={start:pe,received:'',spent:'',end:'',_carried:true,_autoTarget:'',_calcSource:''};else syncCarriedStart(t.gsm[m],pe,tol)} } for(const [m,q] of Object.entries({...t.gsm})){ const pq=source[m],pe=i>0?num(pq?.end):num(pq); if((pe===null||Math.abs(pe)<=tol)&&q?._carried){ const s=num(q.start),r=num(q.received),sp=num(q.spent),e=num(q.end),noMove=(r===null||Math.abs(r)<=tol)&&(sp===null||Math.abs(sp)<=tol),onlyCarry=noMove&&(e===null||s===null||Math.abs(e-s)<=tol||Math.abs(e)<=tol); if(onlyCarry)delete t.gsm[m]; else {q.start='';q._carried=false;if(q._autoTarget==='start')q._autoTarget=''} } } } syncStatementMaterials(st) }
...
export function validateStatement(state, st, p) {
  ...
  trips.forEach((t, i) => {
    ...
    if (v?.hasMotohours) {
      const mhStart = num(t.motohoursStart), mhEnd = num(t.motohoursEnd)
      if (t.unused && mhStart !== null && mhEnd !== null && Math.abs(mhEnd - mhStart) > 0.001) errors.push(`${tag}: неиспользованная путёвка имеет наработку моточасов.`)
      if (mhStart !== null) {
        if (i === 0 && st.opening?.motohours != null && Math.abs(mhStart - Number(st.opening.motohours)) > 0.001) errors.push(`${tag}: начальные моточасы не совпадают с предыдущим периодом.`)
        if (i > 0) {
          const prevMh = motohoursEnd(trips[i - 1])
          if (prevMh !== null && Math.abs(mhStart - prevMh) > 0.001) errors.push(`${tag}: начальные моточасы не совпадают с предыдущей путёвкой.`)
        }
      }
    }
...
export function statementStatus(state,st,p){ const c=validateStatement(state,st,p); if(!st.trips?.length)return{key:'idle',label:'Не ездила',tone:'ok',check:c}; if(c.errors.length)return{key:'bad',label:countForm(c.errors.length,RU_FORMS.error),tone:'bad',check:c}; if(c.warnings.length)return{key:'warn',label:countForm(c.warnings.length,RU_FORMS.warning),tone:'warn',check:c}; return{key:'ok',label:'Готово',tone:'ok',check:c} }
export function stStats(state,st){ let km=0,fuel=0,motohours=0,drivenTrips=0; for(const t of st.trips||[]){const a=num(t.odoStart),b=num(t.odoEnd),mh=motohoursWorked(t);if(a!==null&&b!==null)km+=b-a;if(mh!==null)motohours+=mh;if(!t.unused)drivenTrips++;for(const m of tripMaterialNames(t))if(materialOf(state,m).category==='Топливо')fuel+=num(t.gsm?.[m]?.spent)||0}return{trips:st.trips?.length||0,drivenTrips,km,motohours,fuel} }
...
export function periodFleetRows(state,p){ let cache=PERIOD_ROWS_CACHE.get(state);if(!cache){cache=new WeakMap();PERIOD_ROWS_CACHE.set(state,cache)}const hit=cache.get(p);if(hit)return hit;const byVehicle=new Map((p.statements||[]).map(st=>[st.vehicleId,st]));const rows=(state.vehicles||[]).map(base=>{const v=vehicleOf(state,base.id),st=byVehicle.get(base.id);if(!st)return{vehicle:v,statement:null,status:{key:'idle',label:'Не ездила',tone:'ok'},stats:{trips:0,drivenTrips:0,km:0,motohours:0,fuel:0}};return{vehicle:v,statement:st,status:statementStatus(state,st,p),stats:stStats(state,st)}});cache.set(p,rows);return rows }
