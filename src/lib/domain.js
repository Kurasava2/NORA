...
import { allVehiclesFrom, carriedMotohourReading, motohoursWorked, normalizeCustomVehicles, resolveVehicle, vehicleRateLabel } from './vehicles.js'
...
export const DEFAULT_STATE = { version: STORAGE_VERSION, settings: DEFAULT_SETTINGS, periods: [], vehicleSettings: {}, customVehicles: [], catalog: [] }
...
export function migrateState(input) {
  ...
  s.customVehicles = normalizeCustomVehicles(s.customVehicles, () => `custom_${uid()}`)
  ...
  for (const p of s.periods) {
    ...
    for (const st of p.statements) {
      ...
      for (const t of st.trips) {
        t.gsm = t.gsm || {}
        if (num(t.motohourStart) !== null && num(t.motohourEnd) !== null) t.motohours = round2(num(t.motohourEnd) - num(t.motohourStart))
...
export function vehicleOf(state,id){ return resolveVehicle(BASE_VEHICLES,state,id) }
export function allVehicles(state){ return allVehiclesFrom(BASE_VEHICLES,state) }
export { vehicleRateLabel, motohoursWorked }
...
export function prefilledTrip(state,st,p,editingTrip=null){ if(editingTrip)return clone(editingTrip); const prev=predecessorTrip(st),gsm={},source=prev?(prev.gsm||{}):(st.opening?.gsm||{}); for(const [m,q] of Object.entries(source)){ const end=prev?num(q?.end):num(q); if(end!==null&&nonZero(state,end))gsm[m]={start:end,received:'',spent:'',end:'',_carried:true,_autoTarget:''} } const mhStart=carriedMotohourReading(prev,st.opening); return {id:uid(),seq:Date.now()+Math.random(),date:prev?.date||p.start,number:'',odoStart:prev?(num(prev.odoEnd)??''):(num(st.opening?.odo)??''),odoEnd:prev?(num(prev.odoEnd)??''):(num(st.opening?.odo)??''),motohourStart:mhStart,motohourEnd:mhStart,motohours:'',unused:false,note:'',gsm} }
...
export function statementStatus(state,st,p){ const c=validateStatement(state,st,p),trips=st.trips||[]; if(!trips.length)return{key:'idle',label:'Не ездила',tone:'ok',check:c}; if(c.errors.length)return{key:'bad',label:`${c.errors.length} ошибок`,tone:'bad',check:c}; if(!trips.some(t=>!t.unused))return{key:'idle',label:c.warnings.length?`Не ездила · ${c.warnings.length} замеч.`:'Не ездила',tone:c.warnings.length?'warn':'ok',check:c}; if(c.warnings.length)return{key:'warn',label:`${c.warnings.length} замеч.`,tone:'warn',check:c}; return{key:'ok',label:'Готово',tone:'ok',check:c} }
export function stStats(state,st){ let km=0,fuel=0,motohours=0,drivenTrips=0; for(const t of st.trips||[]){if(!t.unused)drivenTrips++;const a=num(t.odoStart),b=num(t.odoEnd);if(a!==null&&b!==null)km+=b-a;const mh=motohoursWorked(t);if(!t.unused&&mh!==null)motohours+=mh;for(const m of tripMaterialNames(t))if(materialOf(state,m).category==='Топливо')fuel+=num(t.gsm?.[m]?.spent)||0}return{trips:st.trips?.length||0,drivenTrips,km,motohours,fuel} }
...
export function lastBalances(state,st){ const t=st.trips?.[st.trips.length-1],tol=toleranceOf(state?.settings?.tolerance),out={}; for(const m of tripMaterialNames(t)){const e=num(t.gsm?.[m]?.end);if(e!==null&&Math.abs(e)>tol)out[m]=e}return out }
export function periodFleetRows(state,p){ let cache=PERIOD_ROWS_CACHE.get(state);if(!cache){cache=new WeakMap();PERIOD_ROWS_CACHE.set(state,cache)}const hit=cache.get(p);if(hit)return hit;const byVehicle=new Map((p.statements||[]).map(st=>[st.vehicleId,st]));const rows=allVehicles(state).map(v=>{const st=byVehicle.get(v.id);if(!st)return{vehicle:v,statement:null,status:{key:'idle',label:'Не ездила',tone:'ok'},stats:{trips:0,drivenTrips:0,km:0,fuel:0,motohours:0}};return{vehicle:v,statement:st,status:statementStatus(state,st,p),stats:stStats(state,st)}});cache.set(p,rows);return rows }
export function vehicleHistory(state,id){let cache=VEHICLE_HISTORY_CACHE.get(state);if(!cache){cache=new Map();VEHICLE_HISTORY_CACHE.set(state,cache)}if(cache.has(id))return cache.get(id);const rows=[];for(const p of sortedPeriods(state)){const st=(p.statements||[]).find(s=>s.vehicleId===id);if(st)rows.push({period:p,statement:st,status:statementStatus(state,st,p),stats:stStats(state,st),balances:lastBalances(state,st)})}cache.set(id,rows);return rows }
