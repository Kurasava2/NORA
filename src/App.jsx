...
import {
  DEFAULT_STATE, STORAGE_VERSION, addTripMaterial, allVehicles, buildOpening, clone, createStatement, fmtDate,
  kmfmt, materialCellName, materialDisplayName, migrateState, motohoursWorked, nfmt, nonZero,
  ...
  toleranceOf, vehicleHistory, vehicleOf, vehicleRateLabel
} from './lib/domain.js'
...
export class AppErrorBoundary extends React.Component {
  constructor(props) {
    super(props)
    this.state = { error: null, saveError: '' }
  }
  static getDerivedStateFromError(error) { return { error } }
  componentDidCatch(error, info) {
    reportRendererError('react:render', new Error(`${error?.stack || error?.message || error}\n${info?.componentStack || ''}`))
  }
  saveBeforeExit = () => {
    if (!window.__gsmStateReady || !window.desktopAPI?.saveDataSync) return { success: true }
    try { return window.desktopAPI.saveDataSync(window.__gsmStateSnapshot) || { success: false, error: 'Нет ответа от процесса сохранения.' } }
    catch (error) { return { success: false, error: error?.message || String(error) } }
  }
  recover = action => {
    const saved = this.saveBeforeExit()
    if (!saved?.success) { this.setState({ saveError: saved?.error || 'Не удалось сохранить данные.' }); return }
    if (action === 'reload') window.location.reload()
    else window.desktopAPI?.closeWindow?.()
  }
  render() {
    if (!this.state.error) return this.props.children
    return <div className="fatal-screen"><div className="fatal-card"><div className="recovery-icon">!</div><h1>Интерфейс приложения аварийно остановлен</h1><p>Данные не сбрасывались. Техническая ошибка записана в журнал. Перед перезапуском или закрытием приложение попробует сохранить уже загруженную базу.</p><div className="recovery-error">{this.state.error?.message || String(this.state.error)}</div>{this.state.saveError&&<div className="recovery-error">Сохранение не выполнено: {this.state.saveError}</div>}<div className="toolbar recovery-actions"><Button primary onClick={()=>this.recover('reload')}>Перезапустить интерфейс</Button><Button onClick={()=>this.recover('close')}>Закрыть приложение</Button></div></div></div>
  }
}
...
export default function App() {
  ...
  window.__gsmStateSnapshot = state
  window.__gsmStateReady = ready && !loadError
  ...
  const cancelScheduledAutosave = () => {
    ...
    if (saveIdle.current !== null && typeof window.cancelIdleCallback === 'function') window.cancelIdleCallback(saveIdle.current)
    saveIdle.current = null
  }

  const queueSave = snapshot => {
    const request = ++saveRequest.current
    const write = async () => {
      if (request < saveRequest.current) return { success: true, skipped: true }
      if (window.desktopAPI?.isElectron) return window.desktopAPI.saveData(snapshot)
      localStorage.setItem('gsm_vedomosti_v12', JSON.stringify(snapshot))
      return { success: true }
    }
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      saveTimer.current = null
      if (typeof window.requestIdleCallback === 'function') saveIdle.current = window.requestIdleCallback(save, { timeout: 1800 })
      else save()
  ...
  async function saveCurrent(showSuccess = true) {
    ...
    try {
      cancelScheduledAutosave()
      const result = await queueSave(stateRef.current)
      if (!result?.success) throw new Error(result?.error || 'Не удалось сохранить локальную базу.')
      setSaveError('')
  ...
  async function startFreshDatabase() {
    ...
    try {
      cancelScheduledAutosave()
      const result = await queueSave(fresh)
    ...
    return runExclusive('восстановление резервной копии', async () => {
        ...
        if (r?.success && window.confirm('Заменить текущие данные данными из резервной копии?')) {
          try {
            ...
            cancelScheduledAutosave()
            const saved = await queueSave(restored)
  ...
  async function exportPeriod(p) {
    return runExclusive('экспорт периода', async () => {
        ...
        try {
          const models = allVehicles(state).map(v => {
            const st = (p.statements || []).find(s => s.vehicleId === v.id) || { id:`idle_${v.id}`, vehicleId:v.id, materials:[], trips:[], opening:{odo:null,motohours:null,gsm:{}} }
  ...
  async function importMonthBook(file) {
    return runExclusive('импорт Excel', async () => {
        ...
        try {
          ...
          const result = await importMonth(await file.arrayBuffer(), allVehicles(state), state.catalog)
  ...
  return <div className={cx('desktop-root', state.settings.performanceMode !== false && 'performance-mode')}>
    ...
    <main className="main-content">
      ...
      {view.name === 'vehicles' && <VehiclesView state={state} onHistory={goHistory} onAdd={() => setVehicleModal(true)} />}
    ...
    <VehicleModal open={vehicleModal} onClose={() => setVehicleModal(false)} state={state} mutate={mutate} notify={notify} />
...
function HomeView({ state, periods, onNew, onOpen, onImport, onDeletePeriod, onDeleteYear }) {
  ...
  const fleetCount = allVehicles(state).length
  const summaries = useMemo(() => periods.map(p => {
    ...
    const drove = rows.reduce((n,r)=>n+(r.stats.drivenTrips>0?1:0),0)
    return { id:p.id, rows, errors, drove, stood:fleetCount-drove }
  }), [state, periods, fleetCount])
  const summaryById = useMemo(() => new Map(summaries.map(x=>[x.id,x])), [summaries])
  const {road,idle,bad} = useMemo(() => summaries.reduce((a,x) => {
    for(const r of x.rows){if(r.stats.drivenTrips>0)a.road++;else a.idle++;if(r.status.key==='bad')a.bad++}
    return a
  }, {road:0,idle:0,bad:0}), [summaries])
  const groups = useMemo(() => groupPeriods(periods), [periods])
  return <div className="page"><PageHead title="Расчётные периоды" subtitle={`${fleetCount} машин · ${state.catalog.length} типов ГСМ · локальная база`} actions={<><input ref={inputRef} className="hidden-input" type="file" accept=".xlsx,.xlsm" onChange={e=>{const f=e.target.files?.[0];e.target.value='';if(f)onImport(f)}}/><Button onClick={()=>inputRef.current?.click()}>⇧ Создать месяц из книги</Button><Button primary onClick={onNew}>＋ Новый период</Button></>} />
    <div className="kpi-grid"><Kpi value={periods.length} label="месяцев"/><Kpi value={road} label="машино-месяцев с путёвками"/><Kpi value={idle} label="машино-месяцев без выезда" tone="ok"/><Kpi value={bad} label="с ошибками" tone={bad ? 'bad' : undefined}/><Kpi value={fleetCount} label="машин в справочнике"/></div>
...
function PeriodView({ state, period, mutate, onOpen, onExport, onHistory }) {
  ...
  const visible = useMemo(() => rows.filter(r => {
    ...
    if (filter === 'idle') return r.stats.drivenTrips === 0
    return filter === 'all' || r.status.key === filter
  }), [rows, deferredQuery, filter])
  const counts = useMemo(() => rows.reduce((o, r) => { o[r.status.key] = (o[r.status.key] || 0) + 1; if (r.stats.drivenTrips === 0) o.idle = (o.idle || 0) + (r.status.key === 'idle' ? 0 : 1); return o }, {}), [rows])
  ...
  return <div className="page"><PageHead title={periodMonthName(period)} subtitle={`${periodYear(period)} · ${fmtDate(period.start)} — ${fmtDate(period.end)} · контроль автопарка`} actions={<Button onClick={onExport}>Экспорт периода XLSX</Button>} />
    <div className="kpi-grid"><Kpi value={counts.ok || 0} label="готово" tone="ok"/><Kpi value={counts.warn || 0} label="с замечаниями"/><Kpi value={counts.bad || 0} label="с ошибками" tone={(counts.bad || 0) ? 'bad' : undefined}/><Kpi value={counts.idle || 0} label="не ездили" tone="ok"/><Kpi value={fleetCount} label="всего машин"/></div>
...
function StatementView({ state, period, statement, vehicle, mutate, onBack, onHistory, onPreview, onExport, onPrint, notify }) {
  ...
  return <div className="page page-wide"><PageHead title={`${vehicle.model} · ${vehicle.shortNo}`} subtitle={`№ ${vehicle.reg} · ${periodMonthName(period)} ${periodYear(period)} · ${periodName(period)}`} actions={<><Button onClick={onBack}>← К месяцу</Button><Button onClick={onHistory}>История</Button><Button onClick={onPreview}>Предпросмотр</Button><Button onClick={onExport}>XLSX</Button><Button primary onClick={onPrint}>Печать</Button></>} />
    <div className="statement-strip"><div><span>Статус</span><Badge tone={status.tone}>{status.label}</Badge></div><div><span>Норма</span><b>{vehicle.baseRate ? `${nfmt(vehicle.baseRate)} ${vehicleRateLabel(vehicle)}` : 'по документу'}</b></div><div><span>Бак</span><b>{vehicle.tankCapacity ? `${nfmt(vehicle.tankCapacity)} л` : '—'}</b></div><div className="grow"><span>ГСМ в путёвках</span><div className="chip-row">{statementMaterialNames(statement).map(m => <span className="chip" key={m}>{materialDisplayName(m)}</span>)}</div></div></div>
...
function TripModal({ open, trip, onClose, state, period, statement, vehicle, mutate, notify }) {
  ...
  const motohours=num(form.motohourStart)!==null&&num(form.motohourEnd)!==null?num(form.motohourEnd)-num(form.motohourStart):null
  ...
  return <Modal open={open} onClose={requestClose} extraWide title={editingId?`Путёвка №${trip?.number||'—'}`:'Новая путёвка'} subtitle={`${vehicle.model} · ${vehicle.shortNo} · ${periodMonthName(period)} ${periodYear(period)}`}><form onSubmit={saveTrip} className="trip-modal-form">
    <div className="trip-modal-body">{errors.length>0&&<div className="form-errors"><b>Путёвку нельзя сохранить:</b>{errors.map((x,i)=><div key={i}>• {x}</div>)}</div>}
      ...
      <SectionStep no="2" title="Одометр" subtitle="показания без разделителей тысяч"><div className="odo-grid"><Field label="Перед выездом"><input className="input input-number-xl" inputMode="decimal" value={form.odoStart??''} onChange={e=>setTripField('odoStart',e.target.value)}/></Field><Field label="После выезда"><input className="input input-number-xl" inputMode="decimal" value={form.odoEnd??''} onChange={e=>setTripField('odoEnd',e.target.value)}/></Field><div className={cx('km-live',km!==null&&km<0&&'bad')}><span>Пройдено</span><b>{km===null?'—':kmfmt(km)}</b><small>км</small></div></div></SectionStep>
      {vehicle.hasMotohours&&<SectionStep no="2А" title="Моточасы" subtitle="как одометр: показание до, показание после и разница"><div className="odo-grid"><Field label="Перед работой"><input className="input input-number-xl" inputMode="decimal" value={form.motohourStart??''} onChange={e=>setTripField('motohourStart',e.target.value)}/></Field><Field label="После работы"><input className="input input-number-xl" inputMode="decimal" value={form.motohourEnd??''} onChange={e=>setTripField('motohourEnd',e.target.value)}/></Field><div className={cx('km-live',motohours!==null&&motohours<0&&'bad')}><span>Отработано</span><b>{motohours===null?(num(form.motohours)!==null?nfmt(form.motohours):'—'):nfmt(motohours)}</b><small>м/ч</small></div></div>{num(form.motohourStart)===null&&num(form.motohourEnd)===null&&num(form.motohours)!==null&&<div className="notice subtle-notice">Старая запись: сохранено только «отработано {nfmt(form.motohours)} м/ч». Укажите показания до/после при следующем редактировании.</div>}</SectionStep>}
...
function TripTable({ state, period, statement, vehicle, onEdit, onDelete }) {
  ...
  return <div className="table-wrap"><table className="data-table trips-table"><thead><tr><th>Дата</th><th>№</th><th>Одометр до</th><th>После</th><th>Км</th>{vehicle.hasMotohours&&<><th>М/ч до</th><th>После</th><th>М/ч</th></>}<th>ГСМ: расход / остаток</th><th>Примечание</th><th></th></tr></thead><tbody>{statement.trips.map(t=>{const prefix=`Путёвка №${t.number||'?'} (${fmtDate(t.date)})`,errs=allErrors.filter(e=>e.startsWith(prefix)),a=num(t.odoStart),b=num(t.odoEnd),mh=motohoursWorked(t),gsm=tripMaterialNames(t).map(m=>{const q=t.gsm?.[m]||{},spent=num(q.spent),end=num(q.end),parts=[];if(spent!==null&&nonZero(state,spent))parts.push(`−${nfmt(spent)}`);if(end!==null&&nonZero(state,end))parts.push(`ост. ${nfmt(end)}`);return parts.length?<span key={m}><b>{materialCellName(m)}</b> {parts.join(' / ')}</span>:null}).filter(Boolean);return <tr key={t.id} tabIndex={0} role="button" className={cx('clickable hover-row',errs.length&&'row-bad',t.unused&&'row-unused')} onClick={()=>onEdit(t)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onEdit(t)}}}><td><b>{fmtDate(t.date)}</b></td><td><b>{t.number}</b>{t.unused&&<Badge>неисп.</Badge>}</td><td className="num">{kmfmt(t.odoStart)}</td><td className="num">{kmfmt(t.odoEnd)}</td><td className="num"><b>{a!==null&&b!==null?kmfmt(b-a):'—'}</b></td>{vehicle.hasMotohours&&<><td className="num">{nfmt(t.motohourStart)}</td><td className="num">{nfmt(t.motohourEnd)}</td><td className="num"><b>{nfmt(mh)}</b></td></>}<td><div className="trip-gsm">{gsm}</div></td><td>{t.note||'—'}</td><td className="actions">{errs.length?<Badge tone="bad">{errs.length}</Badge>:<Badge tone="ok">✓</Badge>}<Button small danger onClick={e=>{e.stopPropagation();onDelete(t)}}>× Удалить</Button></td></tr>})}</tbody></table></div>
}
...
function VehiclesView({ state, onHistory, onAdd }) {
  const [q,setQ]=useState('')
  const deferredQ=useDeferredValue(q)
  const allRows=useMemo(()=>allVehicles(state),[state.vehicleSettings,state.customVehicles])
  ...
  return <div className="page"><PageHead title="Автомобили" subtitle={`${allRows.length} машин в справочнике`} actions={<Button primary onClick={onAdd}>＋ Новая машина</Button>} /><Card><div className="filters"><input className="input search" placeholder="Поиск машины…" value={q} onChange={e=>setQ(e.target.value)}/></div><div className="table-wrap"><table className="data-table vehicle-table"><thead><tr><th>№</th><th>Модель</th><th>Рег. №</th><th>Норма</th><th>Учёт</th><th>Бак</th><th>ГСМ</th><th></th></tr></thead><tbody>{renderedRows.map(v=><tr key={v.id}><td><b className="mono-lg">{v.shortNo}</b>{String(v.id).startsWith('custom_')&&<Badge tone="blue">своя</Badge>}</td><td>{v.model}</td><td>{v.reg}</td><td>{v.baseRate!==null&&v.baseRate!==undefined?`${nfmt(v.baseRate)} ${vehicleRateLabel(v)}`:'—'}</td><td>{v.hasMotohours?'км + моточасы':'километры'}</td><td>{v.tankCapacity?nfmt(v.tankCapacity)+' л':'—'}</td><td className="muted">выбирается в путёвке</td><td className="actions"><Button small onClick={()=>onHistory(v.id)}>История</Button></td></tr>)}</tbody></table></div></Card></div>
}

function VehicleModal({ open, onClose, state, mutate, notify }) {
  const empty={shortNo:'',model:'',reg:'',baseRate:'',rateUnit:'l100km',tankCapacity:'',hasMotohours:false,normText:''}
  const [form,setForm]=useState(empty),[error,setError]=useState('')
  useEffect(()=>{if(open){setForm(empty);setError('')}},[open])
  const set=(key,value)=>setForm(f=>({...f,[key]:value}))
  const submit=e=>{
    e.preventDefault();setError('')
    const shortNo=String(form.shortNo||'').trim(),model=String(form.model||'').trim(),reg=String(form.reg||'').trim()
    const baseRate=num(form.baseRate),tankCapacity=num(form.tankCapacity)
    if(!shortNo)return setError('Укажите номер машины.')
    if(!model)return setError('Укажите модель машины.')
    if(!reg)return setError('Укажите регистрационный номер.')
    if(allVehicles(state).some(v=>String(v.shortNo).toLowerCase()===shortNo.toLowerCase()))return setError('Машина с таким номером уже есть.')
    if(allVehicles(state).some(v=>String(v.reg).toLowerCase()===reg.toLowerCase()))return setError('Машина с таким регистрационным номером уже есть.')
    if(baseRate===null||baseRate<0)return setError('Укажите корректную норму расхода.')
    if(tankCapacity!==null&&tankCapacity<0)return setError('Вместимость бака не может быть отрицательной.')
    const rateUnit=form.rateUnit==='lMotohour'?'lMotohour':'l100km'
    const hasMotohours=rateUnit==='lMotohour'?true:!!form.hasMotohours;const normText=String(form.normText||'').trim()||`Норма расхода ${nfmt(baseRate)} ${rateUnit==='lMotohour'?'л на моточас':'л на 100 км'}.`;const vehicle={id:`custom_${uid()}`,shortNo,model,reg,baseRate,tankCapacity,rateUnit,hasMotohours,normText,defaultMaterials:[]}
    mutate(next=>{next.customVehicles=Array.isArray(next.customVehicles)?next.customVehicles:[];next.customVehicles.push(vehicle)})
    notify(`Машина ${shortNo} добавлена`);onClose()
  }
  return <Modal open={open} onClose={onClose} wide title="Новая машина" subtitle="Норма расхода может считаться по километрам или по моточасам."><form onSubmit={submit}>{error&&<div className="form-errors modal-form-error">{error}</div>}<div className="modal-body grid-2"><Field label="Номер машины"><input className="input" value={form.shortNo} onChange={e=>set('shortNo',e.target.value)} placeholder="Например, 3099" autoFocus/></Field><Field label="Регистрационный номер"><input className="input" value={form.reg} onChange={e=>set('reg',e.target.value)} placeholder="Например, 3099ВР"/></Field><Field className="span-2" label="Модель"><input className="input" value={form.model} onChange={e=>set('model',e.target.value)} placeholder="Марка / модель машины"/></Field><Field label="Норма расхода"><input className="input" inputMode="decimal" value={form.baseRate} onChange={e=>set('baseRate',e.target.value)} placeholder="Число"/></Field><Field label="Единица нормы"><select className="select" value={form.rateUnit} onChange={e=>setForm(f=>({...f,rateUnit:e.target.value,hasMotohours:e.target.value==='lMotohour'?true:f.hasMotohours}))}><option value="l100km">л / 100 км</option><option value="lMotohour">л / моточас</option></select></Field><Field label="Вместимость бака, л"><input className="input" inputMode="decimal" value={form.tankCapacity} onChange={e=>set('tankCapacity',e.target.value)} placeholder="Необязательно"/></Field><label className="toggle-card"><input type="checkbox" checked={form.rateUnit==='lMotohour'||!!form.hasMotohours} disabled={form.rateUnit==='lMotohour'} onChange={e=>set('hasMotohours',e.target.checked)}/><span><b>Учитывать моточасы</b><small>{form.rateUnit==='lMotohour'?'Обязательно для нормы л/моточас':'Добавить показания моточасов к пробегу'}</small></span></label><Field className="span-2" label="Текст нормы / примечание"><textarea className="textarea" value={form.normText} onChange={e=>set('normText',e.target.value)} placeholder="При необходимости вставьте полный текст нормы расхода"/></Field></div><div className="notice subtle-notice">Для машин с учётом моточасов в путёвке будут поля «до», «после» и автоматически рассчитанное «отработано» — так же, как у одометра.</div><div className="modal-actions"><Button type="button" onClick={onClose}>Отмена</Button><Button primary type="submit">Добавить машину</Button></div></form></Modal>
}
...
function PeriodModal({ open, onClose, state, mutate, onCreated }) {
  ...
  const [month,setMonth]=useState(current),[preset,setPreset]=useState('21'),[dates,setDates]=useState(()=>presetDates(current,'21')),[error,setError]=useState('')
  useEffect(()=>{if(open){setMonth(current);setPreset('21');setDates(presetDates(current,'21'));setError('')}},[open])
  const changePreset=(m,p)=>{setMonth(m);setPreset(p);setError('');if(p!=='manual')setDates(presetDates(m,p))}
  const submit=e=>{e.preventDefault();setError('');if(!dates.start||!dates.end||dates.start>dates.end){setError('Проверьте даты периода.');return}if(state.periods.some(p=>p.start===dates.start&&p.end===dates.end)){setError('Такой период уже существует.');return}const p={id:uid(),reportMonth:month,start:dates.start,end:dates.end,statements:[]};mutate(next=>next.periods.push(p));onCreated(p)}
  return <Modal open={open} onClose={onClose} title="Новый расчётный период" subtitle="Месяц станет именем периода, а даты останутся отдельным диапазоном."><form onSubmit={submit}>{error&&<div className="form-errors modal-form-error">{error}</div>}<div className="modal-body grid-2"><Field label="Отчётный месяц"><input className="input" type="month" value={month} onChange={e=>changePreset(e.target.value,preset)}/></Field><Field label="Схема периода"><select className="select" value={preset} onChange={e=>changePreset(month,e.target.value)}><option value="21">21 предыдущего → 20 текущего</option><option value="26">26 предыдущего → 25 текущего</option><option value="calendar">Календарный месяц</option><option value="manual">Произвольные даты</option></select></Field><Field label="Начало"><DateControl value={dates.start} onChange={v=>{setError('');setDates(d=>({...d,start:v}))}}/></Field><Field label="Окончание"><DateControl value={dates.end} onChange={v=>{setError('');setDates(d=>({...d,end:v}))}}/></Field></div><div className="period-preview"><span>Будет создано</span><b>{periodDisplayName({reportMonth:month,start:dates.start,end:dates.end})}</b><small>{fmtDate(dates.start)} — {fmtDate(dates.end)}</small></div><div className="notice">Для машины с путёвками программа найдёт предыдущую заполненную ведомость и перенесёт одометр и остатки ГСМ. Машины без путёвок автоматически считаются не выезжавшими.</div><div className="modal-actions"><Button type="button" onClick={onClose}>Отмена</Button><Button primary type="submit">Создать период</Button></div></form></Modal>
}

function MaterialModal({ open, onClose, state, mutate, notify }) {
  const [form,setForm]=useState({category:'Топливо',name:'',aliases:''}),[error,setError]=useState('')
  useEffect(()=>{if(open){setForm({category:'Топливо',name:'',aliases:''});setError('')}},[open])
  const submit=e=>{e.preventDefault();const name=form.name.trim();setError('');if(!name){setError('Укажите наименование ГСМ.');return}if(state.catalog.some(x=>x.name.toLowerCase()===name.toLowerCase())){setError('Такой тип уже есть в справочнике.');return}mutate(next=>next.catalog.push({id:`custom_${uid()}`,name,category:form.category,unit:'л',aliases:form.aliases.split(',').map(x=>x.trim()).filter(Boolean),sourceVehicles:[],sourceCount:0,active:true}));notify(`Добавлен тип: ${name}`);onClose()}
  return <Modal open={open} onClose={onClose} title="Новый тип ГСМ"><form onSubmit={submit}>{error&&<div className="form-errors modal-form-error">{error}</div>}<div className="modal-body grid-2"><Field label="Категория"><select className="select" value={form.category} onChange={e=>{setError('');setForm(f=>({...f,category:e.target.value}))}}><option>Топливо</option><option>Масло</option></select></Field><Field label="Единица"><input className="input" value="л" readOnly/></Field><Field className="span-2" label="Наименование"><input className="input" value={form.name} onChange={e=>{setError('');setForm(f=>({...f,name:e.target.value}))}} placeholder='Например, Дт "З" или Масло М-10Г2'/></Field><Field className="span-2" label="Алиасы" hint="Через запятую"><input className="input" value={form.aliases} onChange={e=>setForm(f=>({...f,aliases:e.target.value}))}/></Field></div><div className="modal-actions"><Button type="button" onClick={onClose}>Отмена</Button><Button primary type="submit">Добавить</Button></div></form></Modal>
}
