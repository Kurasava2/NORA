...
function fromSafe(value) {
  if (Array.isArray(value)) return value.map(fromSafe)
  if (value && typeof value === 'object') {
    if (Object.keys(value).length === 1 && value.__gsm_null === true) return null
    const out = {}
    for (const [key, item] of Object.entries(value)) out[key] = fromSafe(item)
    return out
  }
  return value
}
...
function parseLegacyToml(text) {
  ...
  if (!parsed?.data) throw new Error('В старой TOML-копии нет раздела data.')
  return {
    data: fromSafe(parsed.data),
    meta: {
      format: parsed.backup?.format || 'legacy-toml',
      backupFormatVersion: Number(parsed.backup?.schema) || 0,
      createdAt: parsed.backup?.created_at || '',
      appVersion: '',
      dataSchemaVersion: Number(parsed.data?.version) || 0,
      verified: false,
      legacy: true
    }
  }
...
function parseBackup(text, ext = '') {
  const trimmed = String(text || '').trimStart()
  const lowerExt = String(ext || '').toLowerCase()
  if (lowerExt === '.gsmbackup') return parseUniversalBackup(text)

  if (trimmed.startsWith('{')) {
    let parsed
    try { parsed = JSON.parse(text) } catch { throw new Error('JSON резервной копии повреждён.') }
    if (parsed?.format === BACKUP_FORMAT) return parseUniversalBackup(text)
    const plainObject = parsed && typeof parsed === 'object' && !Array.isArray(parsed)
    const recognizableState = plainObject
      && Array.isArray(parsed.periods)
      && (parsed.settings === undefined || (parsed.settings && typeof parsed.settings === 'object' && !Array.isArray(parsed.settings)))
      && (parsed.vehicleSettings === undefined || (parsed.vehicleSettings && typeof parsed.vehicleSettings === 'object' && !Array.isArray(parsed.vehicleSettings)))
      && (parsed.catalog === undefined || Array.isArray(parsed.catalog))
    if (!recognizableState) throw new Error('Выбранный JSON не похож на резервную копию ГСМ Ведомости.')
    return {
      data: parsed,
      meta: {
        format: 'legacy-json',
        ...
        createdAt: '',
        appVersion: '',
        dataSchemaVersion: Number(parsed?.version) || 0,
        verified: false,
        legacy: true
      }
    }
