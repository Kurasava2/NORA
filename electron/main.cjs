...
const { createUniversalBackup, parseBackup } = require('./backup.cjs')
...
ipcMain.handle('backup:import', async () => {
  const out = await dialog.showOpenDialog(mainWindow, {
    ...
    properties: ['openFile'],
    filters: [
      { name: 'ГСМ универсальная копия', extensions: ['gsmbackup'] },
      { name: 'Старые резервные копии', extensions: ['toml', 'json'] }
    ]
  })
  ...
  try {
    ...
    const parsed = parseBackup(text, path.extname(file))
